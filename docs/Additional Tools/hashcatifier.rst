...
`Hashcatifier <https://github.com/InfamousSYN/rogue/blob/master/tools/hashcatifer.py>`_ is a simple tool used to convert captured hashes into a format consumable by hashcat.
...
- ``--format`` - Specify the format to output hashes (currently the only supported format is NET-NTLMv1, ``hashcat -m 5500``).
